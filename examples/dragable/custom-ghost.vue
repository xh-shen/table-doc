<template>
	<s-table :columns="columns" :data-source="dataSource" :pagination="false" allow-column-drag>
		...
		<template #columnDragGhost="{ column, icon, targetColumn }">
			<component :is="icon"></component>
			<span style="color: red">
				dragging from {{ column?.title }} to
				{{ targetColumn?.title }}
			</span>
		</template>
	</s-table>
