...
export function resolveHeaders(headers: MenuItem[], levelsRange: Exclude<DefaultTheme.Config['outline'], false> = 2) {
...
function addToParent(currIndex: number, headers: MenuItem[], levelsRange: [number, number]) {
	if (currIndex === 0) {
		return true
	}
